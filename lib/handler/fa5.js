...
async function loadIcons(cfg, item) {
	...
	let content = await readFile(item.path + '/web-fonts-with-css/less/_variables.less');
...
async function loadFontData(cfg, item, iconList) {
	...
	let filesList = [
		item.path + '/web-fonts-with-css/webfonts/fa-regular-400.svg',
		item.path + '/web-fonts-with-css/webfonts/fa-brands-400.svg',
		item.path + '/web-fonts-with-css/webfonts/fa-solid-900.svg'
	];
